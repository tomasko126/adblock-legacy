  ...
  if (!SAFARI) {
    ...
    const URL_STATS = 'https://hits.disconnect.me';
    ...
    function getPostStatsRequest() {
      ...
      data.path = data.path + [
        ...
        'build=' + localStorage.versionInstalled,
        'search_build=' + localStorage.searchInstalled,
    ...
    function initializeSearch(callback) {
      ...
      localStorage['show_secure_search'] = "false";
      ...
      localStorage.omnibox_on = localStorage.omnibox_off = 0;
      localStorage.everywhere_on = localStorage.everywhere_off = 0;
      localStorage.secure_search_on = localStorage.secure_search_off = 0;
      localStorage.searches_total = localStorage.pitch_page_total = 0;
      localStorage.pitch_page_yes = localStorage.pitch_page_no = 0;
      localStorage.show_search_form = false;

      localStorage.versionInstalled = chrome.app.getDetails().version.toString() || "2.6.18";
      localStorage.searchInstalled = "1.5.0";
      localStorage.search_group = 'gadblock';
      localStorage.search_product = 'adblock';
      localStorage.pitch_page_shown = false;
      callback && callback();
    }
    ...
    chrome.webRequest.onBeforeRequest.addListener(function(details) {
      ...
      if (isDisconnectSearchPage) localStorage.searches_total = parseInt(localStorage.searches_total) + 1;

      // Deactivate Search Mode
      if (localStorage.show_secure_search === "false")
    ...
    chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    	...
    	if (request.action=='notNow') {
        	...
        	localStorage.pitch_page_no = parseInt(localStorage.pitch_page_no) + 1;
    	}
    	else if (request.action=='doEnable') {
    		localStorage.show_secure_search = true;

        	localStorage.pitch_page_yes = parseInt(localStorage.pitch_page_yes) + 1;
    	}
    });
    
    function shouldShowSearchForm() {
    	if (localStorage.show_search_form!=undefined)
    		return localStorage.show_search_form;
    ...
    function reportUsage() {
      ...
      var weeklyPing     = new Date(localStorage.weeklyPing || now);
      var monthlyPing    = new Date(localStorage.monthlyPing || now);
      var quarterlyPing  = new Date(localStorage.quarterlyPing || now);
      var semiannualPing = new Date(localStorage.semiannualPing || now);
      var yearlyPing     = new Date(localStorage.yearlyPing || now);

      var daily      = ((now.getTime() - lastPing.getTime()) >= oneDayAsMsec);
      ...
      if      (yearly)     report_update_type = 0x20 | 0x10 | 0x08 | 0x04 | 0x01;
        ...
        var report_values_to_send = {
          ...
          show_secure_search: localStorage.show_secure_search || false,
          omnibox_on: localStorage.omnibox_on || 0,
          omnibox_off: localStorage.omnibox_off || 0,
          everywhere_on: localStorage.everywhere_on || 0,
          everywhere_off: localStorage.everywhere_off || 0,
          secure_search_on: localStorage.secure_search_on || 0,
          secure_search_off: localStorage.secure_search_off || 0,
          searches_total: localStorage.searches_total || 0,
          pitch_page_total: localStorage.pitch_page_total || 0,
          pitch_page_yes: localStorage.pitch_page_yes || 0,
          pitch_page_no: localStorage.pitch_page_no || 0
        }
        data.path = data.path + '&' + [
        ...
        $.ajax(data.conn, {
          ...
          success: function(data, textStatus, jqXHR) {
            ...
            if (firstUpdate)               localStorage.firstPing      = now;
            if (weekly || firstUpdate)     localStorage.weeklyPing     = now;
            if (monthly || firstUpdate)    localStorage.monthlyPing    = now;
            if (quarterly || firstUpdate)  localStorage.quarterlyPing  = now;
            if (semiannual || firstUpdate) localStorage.semiannualPing = now;
            if (yearly || firstUpdate)     localStorage.yearlyPing     = now;
            localStorage.omnibox_on = parseInt(localStorage.omnibox_on) - report_values_to_send.omnibox_on;
            localStorage.omnibox_off = parseInt(localStorage.omnibox_off) - report_values_to_send.omnibox_off;
            localStorage.everywhere_on = parseInt(localStorage.everywhere_on) - report_values_to_send.everywhere_on;
            localStorage.everywhere_off = parseInt(localStorage.everywhere_off) - report_values_to_send.everywhere_off;
            localStorage.secure_search_on = parseInt(localStorage.secure_search_on) - report_values_to_send.secure_search_on;
            localStorage.secure_search_off = parseInt(localStorage.secure_search_off) - report_values_to_send.secure_search_off;
            localStorage.searches_total = parseInt(localStorage.searches_total) - report_values_to_send.searches_total;
            localStorage.pitch_page_total = parseInt(localStorage.pitch_page_total) - report_values_to_send.pitch_page_total;
            localStorage.pitch_page_yes = parseInt(localStorage.pitch_page_yes) - report_values_to_send.pitch_page_yes;
            localStorage.pitch_page_no = parseInt(localStorage.pitch_page_no) - report_values_to_send.pitch_page_no;
          }
        });
    };
    ...
    function showPitchPage() {
        TABS.create({url: localStorage.search_group_pitch}, function(tab) {
        	
        	var myid = chrome.i18n.getMessage("@@extension_id");
        	var code = "document.getElementById('noPrivateSearch').addEventListener(\"click\", function() { chrome.runtime.sendMessage(\"" + myid + "\", {action:\"notNow\"}); });";
            code += "document.getElementById('yesPrivateSearch').addEventListener(\"click\", function() { chrome.runtime.sendMessage(\"" + myid + "\",{action:\"doEnable\"}); });";
        	chrome.tabs.executeScript(tab.id, {code: code, allFrames: false, runAt: "document_end"});
        	
        	localStorage.pitch_page_shown = true;
    		localStorage.show_search_form = true;
        	localStorage.pitch_page_total = parseInt(localStorage.pitch_page_total) + 1;
        });
    };
    
    // if we don't yet have a golden ticket, try to get one on every startup
    var grp = localStorage.search_group;
    var newInstallt = deserialize(localStorage['newInstallt']);
    if (grp == undefined || grp == 'gadblock') {

        
        $.getJSON('https://goldenticket.disconnect.me/goldenticket/ticket/fetch?product=AdBlock', function(data) {
        	...
        	if (data['test']!=undefined && data['test']!='false') {
              ...
              localStorage.search_group = data['test'];
          		...
          		// only show the pitch page if this isn't a new install
    ...
    else if (localStorage.search_group_pitch!=undefined){
    	if (localStorage.pitch_page_shown==false || localStorage.pitch_page_shown=="false") {
        	showPitchPage();
    	}
      if (typeof newInstallt === 'undefined') {
        localStorage['newInstallt'] = "false";
